use crate::{
    AtmosphereRenderer, BloomEffect, BrainSystem, Camera, CameraController, ColorCorrectionEffect,
    ColorCorrectionUniform, ImguiState, Input, Model, Transform, VoxelRenderer,
};
...
impl<'window> WgpuCtx<'window> {
    ...
    pub async fn new_async(window: Arc<Window>) -> Self {
        ...
        // Initialize Neuron System
        let brain_system = BrainSystem::new(
            Arc::clone(&device),
            Arc::clone(&queue),
            &camera_bind_group_layout,
            &shader_hot_reload,
            "https://192.168.50.208:8080".to_string(),
        );
        ...
        Self {
            ...
            imgui,
            brain_system,
            render_mode: RenderMode::NeuralNet,
            voxel_renderer,
            atmosphere_renderer,
    ...
    pub fn draw(&mut self, world: &mut World, window: &Window, input: &Input) {
        ...
        match self.render_mode {
            ...
            RenderMode::NeuralNet => {
                // 1. Explicit Clear Pass
                // Since BrainSystem::render uses LoadOp::Load (to allow overlaying),
                // we must manually clear the screen and depth buffer first.
                {
                ...
                } // End of clear pass (drops mutable borrow of encoder)

                // 2. Update Brain Simulation (CPU -> GPU Uploads)
                // Uploads time, mouse coords, and new camera frame if available
                let window_size = (self.surface_config.width, self.surface_config.height);
                self.brain_system.update(input, window_size);

                // 3. Render Brain (Compute Simulation -> Draw Instances)
        ...
        ui.window("Settings")
            ...
            .build(|| {
                ...
                match self.render_mode {
                    ...
                    RenderMode::NeuralNet => {
                        // if self.render_mode == RenderMode::NeuralNet {
                        //     ui.separator();
                        //     ui.text("Neural Simulation");
                        //     ui.slider(
                        //         "Learning Rate",
                        //         0.0,
                        //         0.1,
                        //         &mut self.neuron_system.params.learning_rate,
                        //     );
                        //     ui.slider(
                        //         "Interaction Radius",
                        //         0.0,
                        //         0.5,
                        //         &mut self.neuron_system.params.interaction_radius,
                        //     );
                        //     ui.text(format!("Count: {}", self.neuron_system.params.count));
                        // }
                    }
                }
